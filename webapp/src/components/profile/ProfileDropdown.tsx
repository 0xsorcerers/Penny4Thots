    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="group relative flex items-center gap-2 rounded-xl bg-card/80 backdrop-blur-sm border border-border/50 px-4 py-2.5 font-outfit font-medium text-foreground transition-all hover:border-primary/30 hover:bg-card hover:shadow-[0_0_20px_rgba(var(--primary),0.1)]"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <DropdownMenuContent
            align="end"
            sideOffset={8}
            className="w-72 rounded-2xl border-border/50 bg-card/95 backdrop-blur-xl p-2 shadow-2xl"
            asChild
          >
            <motion.div
