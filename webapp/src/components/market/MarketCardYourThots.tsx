...
export function MarketCardYourThots({ market, onVoteClick }: MarketCardYourThotsProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative cursor-pointer overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-950/30 via-card/80 to-purple-950/30 backdrop-blur-sm transition-all duration-300 hover:border-violet-500/40 hover:shadow-[0_0_30px_rgba(139,92,246,0.15)]"
    >
      ...
      <div className="absolute inset-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-violet-950/90 via-card/80 to-purple-950/50" />
      </div>

      {/* Voted Badge */}
      <div className="absolute top-4 right-4 z-20">
        <div className="flex items-center gap-1.5 rounded-full bg-violet-500/20 px-2.5 py-1 border border-violet-500/30">
          <Vote className="h-3 w-3 text-violet-400" />
          <span className="font-mono text-xs text-violet-400">{t(selectedLanguage, "marketCard.voted")}</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col p-5">
        ...
        <div className="mb-3 flex flex-wrap gap-1.5">
          {market.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="rounded-full bg-violet-500/15 px-2.5 py-0.5 font-mono text-xs text-violet-400"
            >
              {tag}
            </span>
          ))}
          {market.tags.length > 3 && (
            <button
              ...
              className="rounded-full bg-violet-500/10 px-2.5 py-0.5 font-mono text-xs text-violet-400/70 transition-colors hover:bg-violet-500/20 hover:text-violet-400"
            >
        ...
        {/* Progress bar showing option A/B ratio with violet theme */}
        <div className="mb-4">
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-purple-500/30">
            <motion.div
        ...
        {!market.closed ? (
          (() => {
            ...
            const buttonText = timerExpired
              ...
              : t(selectedLanguage, "marketCard.voteAgain");
          ...
          <div className="flex w-full items-center justify-center gap-2 rounded-xl py-2.5 bg-gradient-to-r from-slate-500/10 to-gray-500/10 border border-slate-500/20">
            ...
            <span className="font-outfit text-xs text-slate-600 dark:text-slate-400">{t(selectedLanguage, "marketCard.resolving")}</span>
          </div>
        ) : isLoadingPositions ? (
          <div className="flex w-full items-center justify-center gap-2 rounded-xl py-2.5 bg-muted/30">
            <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
          ...
          <motion.button
            ...
            onClick={handleClaim}
            disabled={isClaiming}
            className="flex w-full items-center justify-center gap-2 rounded-xl py-2.5 font-outfit text-sm font-medium transition-all bg-violet-600/25 text-violet-300 hover:bg-violet-600/35 border border-violet-500/40 hover:border-violet-500/60 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isClaiming ? (
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="pointer-events-none absolute inset-0 rounded-2xl ring-1 ring-inset ring-violet-500/30"
          />
        )}
      </AnimatePresence>
      ...
      <Dialog open={showAllTags} onOpenChange={setShowAllTags}>
        <DialogContent className="max-w-md bg-card/95 backdrop-blur-sm border-violet-500/30">
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {market.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-violet-500/15 px-3 py-1.5 font-mono text-sm text-violet-400"
                >
                  {tag}
                </span>
              ))}
